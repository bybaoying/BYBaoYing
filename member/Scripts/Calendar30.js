...
function calendar(vObj, e) //主调函数
{
 
//    var e = window.event ? window.event.srcElement:vObj;
//    var e = (event == null)?window.event:event;

            var evt =  e  ||  event;
            var src = evt.srcElement || evt.target;  // 获取触发事件的源对象
             
//    var e = window.event.srcElement;  //ImageButton1
    writeIframe();
    var o = WebCalendar.calendar.style; WebCalendar.eventSrc = src;
	if (arguments.length == 0) WebCalendar.objExport = src;
    else WebCalendar.objExport = eval(arguments[0]);//vObj =arguments[0]

    WebCalendar.iframe.tableWeek.style.cursor = WebCalendar.drag ? "move" : "default";
	var t = src.offsetTop,  h = src.clientHeight, l = src.offsetLeft, p = src.type;
	while (src = src.offsetParent){t += src.offsetTop; l += src.offsetLeft;}
    o.display = ""; 
//    WebCalendar.iframe.document.body.focus();// ie 好用，ff 不行
...
function writeIframe()
{
    ...
    "</style></head>\r\n<body onselectstart='return false' style='margin: 0px' oncontextmenu='return false'><form name=meizz>";

    if (WebCalendar.drag){ strIframe += "\r\n<scr"+"ipt language=javascript>\r\n"+
    "var drag=false, cx=0, cy=0, o = parent.WebCalendar.calendar;\r\n"+
    "\r\n" + 
    " document.onmousemove=function(e) { var evt =  e  ||  event;\r\n"+
    "if(parent.WebCalendar.drag && drag){if(o.style.left=='')o.style.left=0; if(o.style.top=='')o.style.top=0;"+
    "o.style.left = parseInt(o.style.left) + evt.clientX-cx;"+
    "o.style.top  = parseInt(o.style.top)  + evt.clientY-cy;}} \r\n"+
    " document.onkeydown= function(e) { var evt =  e  ||  event;  switch(evt.keyCode){  case 27 : parent.hiddenCalendar(); break;"+
    "case 37 : parent.prevM(); break; case 38 : parent.prevY(); break; case 39 : parent.nextM(); break; case 40 : parent.nextY(); break;"+
    "case 84 : document.forms[0].today.click(); break;} evt.keyCode = 0; evt.returnValue= false;}\r\n"+
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         for(var y=0; y<7; y++)  strIframe += "<td class=out id='meizzDay"+ (x*7+y) +"' onclick=\"javascript:parent.returnDate('" + (x*7+y) + "');\"></td>"; strIframe += "</tr>";}
         strIframe += "<tr>";
         for(var x=35; x<39; x++) strIframe += "<td class=out id='meizzDay"+ x +"' onclick=\"javascript:parent.returnDate('" + (x) + "');\"></td>";
    ...
    "    <tr><td><input  style='width: 20' name=prevYear title='向前翻 1 年&#13;快捷键：↑' onclick='parent.prevY()' type=button value='&lt;&lt;'"+
    "    onfocus='this.blur()' style='meizz:expression(this.disabled=parent.WebCalendar.thisYear==1000)'><input   style='width: 20'"+
    "    onfocus='this.blur()' name=prevMonth title='向前翻 1 月&#13;快捷键：←' onclick='parent.prevM()' type=button value='&lt;&nbsp;'>"+
    "    </td><td  valign=top align=center><input id='buttoday' name=today type=button value='今天' onfocus='this.blur()' style='width: 40' title='当前日期&#13;快捷键：T'"+
    "    onclick=\"parent.returnDate(new Date().getDate() +'/'+ (new Date().getMonth() +1) +'/'+ new Date().getFullYear())\">"+
    "    </td><td align=right><input   style='width: 20' title='向后翻 1 月&#13;快捷键：→' name=nextMonth onclick='parent.nextM()' type=button value='&nbsp;&gt;'"+
    "    onfocus='this.blur()'><input  style='width: 20' name=nextYear title='向后翻 1 年&#13;快捷键：↓' onclick='parent.nextY()' type=button value='&gt;&gt;'"+
...
//    this.iframe     = window.frames("meizzCalendarIframe"); //日历的 iframe 载体
    this.iframe     = window.meizzCalendarIframe; //日历的 iframe 载体
...
//    this.style.removeAttribute('backgroundColor');
    this.style.removeAttribute?this.style.removeAttribute('backgroundColor'):this.style.removeProperty('background-color');
 
//    this.style.backgroundColor = "";//这个也好用
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    WebCalendar.iframe.meizzYearHead.innerHTML  = y +" 年";
    WebCalendar.iframe.meizzYearMonth.innerHTML = parseInt(m, 10) +" 月";
...
//        WebCalendar.dayObj[i].onclick     = function(){  returnDate("" ) };
//        WebCalendar.dayObj[i].attachEvent ("onclick", "returnDate('" +  WebCalendar.dayObj[i].title + "' )");
...
function returnDate(vIndex) //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        var returnValue;
//        var a = (arguments.length==0) ? WebCalendar.day[this.id.substr(8)].split("/") : arguments[0].split("/");
//arguments 获取今天的参数，点今天有，点日期无参数，在ff 中没有参数传递都有，arguments.length =1
        var a = (arguments[0].length<3) ?  WebCalendar.day[vIndex].split("/") : arguments[0].split("/");
 ...
 document.onclick=function (e)
{
    var evt =  e  ||  event;
    var src = evt.srcElement || evt.target;  // 获取触发事件的源对象
    if(WebCalendar.eventSrc != src) hiddenCalendar();
}
